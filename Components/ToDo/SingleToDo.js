...
export default function SingleToDo(props) {
    ...
    const flipDone = () => {
        ...
        axios.put(`http://todoapi.brodishrout.com/api/todo/${props.todo.toDoId}`, updatedToDo).then(response => {
    ...
    const deleteToDo = (id) => {
        if(window.confirm(`Are you sure you want to delete ${props.todo.name}?`)) {
            axios.delete(`http://todoapi.brodishrout.com/api/todo/${id}`).then(() => {props.getToDo()})
        }
    }
